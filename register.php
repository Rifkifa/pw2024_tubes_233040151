<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if ($conn->query($sql)){
            $register_message = "Akun berhasil didaftarkan, silahkan login";
        } else {
            $register_message = "Username atau Email sudah terdaftar";
        }
    }
    $conn->close();
?>
...
<body>
    ...
    <main class="d-flex align-items-center" style="height: 70vh;">
        <div class="container py-5 text-center">
            <h3 class="pt-5 text-center">Daftarkan Akun</h3>
            <i><?= $register_message ?></i>
            <div class="container d-flex justify-content-center">
                <form action="register.php" method="POST" class="text-center">
                    <label for="username">Username: </label>
                    <div class="username mb-4">    
                        <input type="text" class="text-center" placeholder="Username" name="username" size="40" style="height:40px;" autocomplete="off"/>
                    </div>
                    ...
                    <button type="submit" name="submit" class="btn btn-primary" style="width: 263px; height:40px;">Sign Up</button>
                </form>
            </div>
        </div>
    </main>
